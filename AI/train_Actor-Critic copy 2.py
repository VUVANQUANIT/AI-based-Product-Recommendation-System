...
class ActorCritic:
    def __init__(self, state_dim, action_dim):
        ...
        self.tau = 0.005
        self.best_loss = float('inf')  # Theo dõi loss tốt nhất để lưu checkpoint
    ...
    def train_batch(self, batch_size=32, gamma=0.9):
        ...
        if critic_loss < self.best_loss:
            self.best_loss = critic_loss
            self.save()
            logging.info(f"Đã lưu mô hình với critic_loss={critic_loss:.4f}")
    ...
    def save(self):
        # Lưu cả actor, critic và target networks
        self.actor.save("actor.keras")
        self.critic.save("critic.keras")
        self.target_actor.save("target_actor.keras")
        self.target_critic.save("target_critic.keras")
        logging.info("Đã lưu mô hình Actor-Critic và target networks")

    def load(self):
        try:
            # Kiểm tra sự tồn tại của file mô hình
            if os.path.exists("actor.keras") and os.path.exists("critic.keras"):
                self.actor = tf.keras.models.load_model("actor.keras")
                self.critic = tf.keras.models.load_model("critic.keras")
                # Tải target networks nếu tồn tại
                if os.path.exists("target_actor.keras") and os.path.exists("target_critic.keras"):
                    self.target_actor = tf.keras.models.load_model("target_actor.keras")
                    self.target_critic = tf.keras.models.load_model("target_critic.keras")
                else:
                    self.target_actor = tf.keras.models.clone_model(self.actor)
                    self.target_critic = tf.keras.models.clone_model(self.critic)
                    self.target_actor.set_weights(self.actor.get_weights())
                ...
                self.actor_optimizer = tf.keras.optimizers.Adam(learning_rate=0.001)
                self.critic_optimizer = tf.keras.optimizers.Adam(learning_rate=0.001)
                logging.info("Đã tải mô hình Actor-Critic và target networks")
            else:
                logging.warning("Không tìm thấy file mô hình, giữ mô hình hiện tại")
        except Exception as e:
            logging.error(f"Lỗi khi tải mô hình: {e}, giữ mô hình hiện tại")

update_global_data()
model = ActorCritic(global_data["state_dim"], global_data["action_dim"])
model.load()  # Tải mô hình ngay khi khởi tạo để sử dụng trọng số cũ nếu có
...
def background_training():
    ...
    while True:
        time.sleep(300)
        update_global_data()
        current_state_dim = model.actor.input_shape[1]
        current_action_dim = model.actor.output_shape[1]
        
        if global_data["state_dim"] != current_state_dim or global_data["action_dim"] != current_action_dim:
            logging.info("Kích thước thay đổi, chuyển trọng số sang mô hình mới")
            old_actor_weights = model.actor.get_weights()
            old_critic_weights = model.critic.get_weights()
            model = ActorCritic(global_data["state_dim"], global_data["action_dim"])
            try:
                # Ánh xạ trọng số cũ nếu tương thích
                model.actor.set_weights(old_actor_weights)
                model.critic.set_weights(old_critic_weights)
                model.target_actor.set_weights(old_actor_weights)
                model.target_critic.set_weights(old_critic_weights)
                logging.info("Đã chuyển trọng số cũ sang mô hình mới")
            except:
                logging.warning("Không thể chuyển trọng số, khởi tạo mô hình mới")
            model.load()  # Tải checkpoint nếu có
...
def update_policy():
    try:
        ...
        if user_id in global_data["user_id_to_index"] and product_id in global_data["product_id_to_index"]:
            user_idx = global_data["user_id_to_index"][user_id]
            product_idx = global_data["product_id_to_index"][product_id]
            category_id = None  # Giả định lấy từ DB nếu cần
...
def recommend():
    try:
        ...
        model.load()  # Tải mô hình trước khi dự đoán
